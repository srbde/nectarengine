...
class Token(dict):
    ...
    def __init__(self, symbol: str | dict[str, Any], api: Api | None = None) -> None:
        if api is None:
            self.api = Api()
        else:
            self.api = api
    ...
    def quantize(self, amount: float | int | str) -> decimal.Decimal:
    ...
    def get_market_info(self) -> dict[str, Any] | None:
